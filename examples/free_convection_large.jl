...
simulation = Simulation(model, Δt=1.0, stop_time=8hour)
...
w2_budget_statistics  = LESbrary.TurbulenceStatistics.uiuj_budget(model, terms = (:w, :w))
wT_budget_statistics  = LESbrary.TurbulenceStatistics.uic_budget(model, terms = (:w, :T))

simulation.output_writers[:statistics] =
    JLD2OutputWriter(model, merge(turbulence_statistics, tke_budget_statistics, w2_budget_statistics, wT_budget_statistics),
                     schedule = AveragedTimeInterval(1hour, window=15minute),
