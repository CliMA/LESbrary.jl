...
function parse_command_line_arguments()
    ...
    @add_arg_table! settings begin
        ...
        "--output_dimensions"
            help = "The spatial dimensions of the output data. Choose between 3 or 1."
            default = 1
            arg_type = Int
    end
...
output_dims = args["output_dimensions"]
f = args["coriolis"]
name = args["name"]
...
function (hha::HackyHorizontalAverage)(model)
    ...
    return interior_copy(F)
end

struct HackyProductHorizontalAverage{F, G, H}
    field1 :: F
    field2 :: G
    surface_flux :: H
end
...
function (hpha::HackyProductHorizontalAverage)(model)
    f = hpha.field1
    g = hpha.field2
    surface_flux = hpha.surface_flux
    ...
    # inject_surface_flux!(FG, surface_flux, model.clock.time)
    return interior_copy(FG)
end

inject_surface_flux!(field, flux, t) = nothing  # fallback
inject_surface_flux!(field, flux::Number, t) = CUDA.@allowscalar (field[:, :, field.grid.Nz+1] .= flux)
inject_surface_flux!(field, flux::Function, t) = CUDA.@allowscalar (field[:, :, field.grid.Nz+1] .= flux(0, 0, t))
...
statistics_to_output = (
    ...
    wu = HackyProductHorizontalAverage(w, u, Qᵘ),
    wv = HackyProductHorizontalAverage(w, v, nothing),
    wT = HackyProductHorizontalAverage(w, T, Qᶿ)
)
