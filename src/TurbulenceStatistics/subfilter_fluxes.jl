...
function subfilter_momentum_fluxes(model;
    ...
    averages = Dict(
                    :νₑ_∂z_u => AveragedField(∂z(u) * νₑ, dims=(1, 2), operand_data=w_scratch),
                    :νₑ_∂z_v => AveragedField(∂z(v) * νₑ, dims=(1, 2), operand_data=w_scratch),
                    :νₑ_∂z_w => AveragedField(∂z(w) * νₑ, dims=(1, 2), operand_data=c_scratch),
                   )
...
function subfilter_tracer_fluxes(model; w_scratch=ZFaceField(model.architecture, model.grid))
    ...
    for tracer in propertynames(model.tracers)
        ...
        averages[name] = AveragedField(∂z(c) * κₑ, dims=(1, 2), operand_data=w_scratch)
    end
